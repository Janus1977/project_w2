...
class database {
    ...
    public function connexion($pooling=false) {
        if (empty($this->_dbDns) || empty($this->_dbUser) || empty($this->_dbPassword)) {
            throw new exceptionDatabase("Param&egrave;tre de connexion manquant");
        } else {
    ...
    public function executeRequete($requete="") {
        ...
        //Mémmorise le nombre de lignes affectees
        $this -> _affectedLinesCount = self::$_instance -> _statement -> rowCount();
...
/////////
    ...
    public function rowCount() {
        return (int) $this -> _affectedLinesCount;
    }
