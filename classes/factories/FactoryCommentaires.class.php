...
abstract class FactoryCommentaires {
	...
	public static function getFromTableCommentaires($id=-1) {
		...
		if (! database::getInstance() -> executeRequete()) {
			throw new Exception(__CLASS__.'::'.__FUNCTION__.'(): Impossible de lire la table commentaires');
		}
	 ...
	 * Il sera preserve lors de la reconstruction automatique.
	 */
	/*[TAG-Commentaires2]*/	/*[/TAG-Commentaires2]*/

		/* Traitement des donnees */
		foreach ($datas AS $data) {
			/* objet par defaut */
			$listeObjet[] = new Commentaires($data['id'],$data['news'],$data['membre'],$data['contenu'],$data['publication'],$data['modification'],$data['valide']);
		
	/*
	 * Entre ces deux balises vous pourrez mettre votre code specifique a la classe.
	 * Il sera preserve lors de la reconstruction automatique.
	 */
	/*[TAG-getFromTableCommentaires1]*/	/*[/TAG-getFromTableCommentaires1]*/
	...
	public static function getFromExtTableNews($news=0) {
		//Generated by FactoryGenerator::generateGetFromTableFromFK()
		$listeObjet = array();
		// Lancement de la requete
		$requete = 'SELECT * FROM `commentaires` ';
		if ($news == 0) {
			$requete .= 'WHERE news > 0 ORDER BY id ASC';
		} else {
			$requete .= 'WHERE news = :news ORDER BY id ASC';
			//Il faut que le parametre soit dans un array pour le BIND
			$news = array(':news' => $news);
		}
		database::getInstance() -> prepareRequete($requete);
		if (is_array($news) || $news > 0) {
			database::getInstance() -> bind($news);
		}
		if (! database::getInstance() -> executeRequete()) {
			throw new Exception(__CLASS__.'::'.__FUNCTION__.'(): Impossible de lire la table commentaires');
		}
