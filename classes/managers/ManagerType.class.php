...
class ManagerType {
	/** Instance de la classe (managerType) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Type) */
	private static $_aListeType = array();
	...
	public function getById($value=-1) {
		...
		return FactoryType::getFromTableType($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryType::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Type) {
			$oType = new Type($object['id'],$object['nom'],$object['description']);
		} else {
			$oType = $object;
		}
		...
		if (database::getInstance()->executeRequete($oType->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Type)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['type']) && sizeof($_SESSION['type']->compareTo($oType)) > 0) {
			$_SESSION['type'] = $oType;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oType->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oType->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Type();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeType); $i++) {
			$object = $this -> _aListeType[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Type de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getTypeVide() {
	...
	public function getByType($nomType) {
		return FactoryType::getFromTableTypeByName($nomType);
	}
