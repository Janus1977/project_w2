...
class ManagerDroit {
	/** Instance de la classe (managerDroit) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Droit) */
	private static $_aListeDroit = array();

	/*
	 * Entre ces deux balises vous pourrez mettre votre code specifique a la classe.
	 * Il sera preserve lors de la reconstruction automatique.
	 */
	/*[TAG-Droit11]*/	/*[/TAG-Droit11]*/
	...
	public function getById($value=-1) {
		...
		return FactoryDroit::getFromTableDroit($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryDroit::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Droit) {
			$oDroit = new Droit($object['id'],$object['nom'],$object['valeur']);
		} else {
			$oDroit = $object;
		}
		...
		if (database::getInstance()->executeRequete($oDroit->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Droit)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['droit']) && sizeof($_SESSION['droit']->compareTo($oDroit)) > 0) {
			$_SESSION['droit'] = $oDroit;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oDroit->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oDroit->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Droit();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeDroit); $i++) {
			$object = $this -> _aListeDroit[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Droit de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getDroitVide() {
