...
class ManagerToto {
	/** Instance de la classe (managerToto) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Toto) */
	private static $_aListeToto = array();
	...
	public function getById($value=-1) {
		...
		return FactoryToto::getFromTableToto($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryToto::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Toto) {
			$oToto = new Toto($object['id'],$object['nom'],$object['prenom']);
		} else {
			$oToto = $object;
		}
		...
		if (database::getInstance()->executeRequete($oToto->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Toto)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['toto']) && sizeof($_SESSION['toto']->compareTo($oToto)) > 0) {
			$_SESSION['toto'] = $oToto;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oToto->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oToto->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Toto();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeToto); $i++) {
			$object = $this -> _aListeToto[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Toto de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getTotoVide() {
