...
class ManagerPermission {
	/** Instance de la classe (managerPermission) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Permission) */
	private static $_aListePermission = array();
	...
	public function getById($value=-1) {
		...
		return FactoryPermission::getFromTablePermission($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryPermission::getFromExtTableJoueur($value);
	}
	...
	public function getFromExtTablePrivilege($idprivilege=-1) {
		...
		return FactoryPermission::getFromExtTablePrivilege($idprivilege);
	}

	public function deleteCompositeLinksFromPrivilege($idprivilege) {
		//Generated by ManagerGenerator::generateDeleteCompositeLinks()
		$requete = 'DELETE FROM permission WHERE privilege = '.$idprivilege;
	}

	public function getFromExtTableGroupe($idgroupe=-1) {
		//Generated by ManagerGenerator::generateGetFromTableFromFK()
		/* Appel de la methode de la Fabrique */
		return FactoryPermission::getFromExtTableGroupe($idgroupe);
	}

	public function deleteCompositeLinksFromGroupe($idgroupe) {
		//Generated by ManagerGenerator::generateDeleteCompositeLinks()
		$requete = 'DELETE FROM permission WHERE groupe = '.$idgroupe;
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Permission) {
			$oPermission = new Permission($object['id'],$object['privilege'],$object['groupe']);
		} else {
			$oPermission = $object;
		}
		...
		if (database::getInstance()->executeRequete($oPermission->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Permission)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['permission']) && sizeof($_SESSION['permission']->compareTo($oPermission)) > 0) {
			$_SESSION['permission'] = $oPermission;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oPermission->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oPermission->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListePermission); $i++) {
			$object = $this -> _aListePermission[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Permission de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getPermissionVide() {
