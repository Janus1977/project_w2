...
class ManagerTest {
	/** Instance de la classe (managerTest) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Test) */
	private static $_aListeTest = array();
	...
	public function getById($value=-1) {
		...
		return FactoryTest::getFromTableTest($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryTest::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Test) {
			$oTest = new Test($object['id'],$object['pseudo'],$object['mail']);
		} else {
			$oTest = $object;
		}
		...
		if (database::getInstance()->executeRequete($oTest->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Test)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['test']) && sizeof($_SESSION['test']->compareTo($oTest)) > 0) {
			$_SESSION['test'] = $oTest;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oTest->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oTest->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Test();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeTest); $i++) {
			$object = $this -> _aListeTest[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Test de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getTestVide() {
