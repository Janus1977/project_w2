...
class ManagerTemp {
	/** Instance de la classe (managerTemp) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Temp) */
	private static $_aListeTemp = array();
	...
	public function getById($value=-1) {
		...
		return FactoryTemp::getFromTableTemp($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryTemp::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Temp) {
			$oTemp = new Temp($object['id'],$object['pseudo'],$object['password'],$object['email'],$object['adresse_ip'],$object['date_inscription'],$object['date_suppression'],$object['checksum']);
		} else {
			$oTemp = $object;
		}
		...
		if (database::getInstance()->executeRequete($oTemp->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Temp)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['temp']) && sizeof($_SESSION['temp']->compareTo($oTemp)) > 0) {
			$_SESSION['temp'] = $oTemp;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oTemp->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oTemp->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Temp();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeTemp); $i++) {
			$object = $this -> _aListeTemp[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Temp de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getTempVide() {
