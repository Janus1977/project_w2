...
class ManagerCamp {
	/** Instance de la classe (managerCamp) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Camp) */
	private static $_aListeCamp = array();
	...
	public function getById($value=-1) {
		...
		return FactoryCamp::getFromTableCamp($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryCamp::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Camp) {
			$oCamp = new Camp($object['id'],$object['nom'],$object['description'],$object['attaque'],$object['sauvegarde'],$object['initiative'],$object['toucher'],$object['pdv']);
		} else {
			$oCamp = $object;
		}
		...
		if (database::getInstance()->executeRequete($oCamp->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Camp)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['camp']) && sizeof($_SESSION['camp']->compareTo($oCamp)) > 0) {
			$_SESSION['camp'] = $oCamp;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oCamp->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oCamp->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Camp();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeCamp); $i++) {
			$object = $this -> _aListeCamp[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Camp de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getCampVide() {
