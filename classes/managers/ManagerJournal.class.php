...
class ManagerJournal {
	/** Instance de la classe (managerJournal) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Journal) */
	private static $_aListeJournal = array();
	...
	public function getById($value=-1) {
		...
		return FactoryJournal::getFromTableJournal($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryJournal::getFromExtTableJoueur($value);
	}

	public function getFromExtTableIdmembre1($ididmembre1=-1) {
		//Generated by ManagerGenerator::generateGetFromTableFromFK()
		/* Appel de la methode de la Fabrique */
		return FactoryJournal::getFromExtTableIdmembre1($ididmembre1);
	}

	public function deleteCompositeLinksFromIdmembre1($ididmembre1) {
		//Generated by ManagerGenerator::generateDeleteCompositeLinks()
		$requete = 'DELETE FROM journal WHERE idmembre1 = '.$ididmembre1;
	}

	public function getFromExtTableIdmembre2($ididmembre2=-1) {
		//Generated by ManagerGenerator::generateGetFromTableFromFK()
		/* Appel de la methode de la Fabrique */
		return FactoryJournal::getFromExtTableIdmembre2($ididmembre2);
	}

	public function deleteCompositeLinksFromIdmembre2($ididmembre2) {
		//Generated by ManagerGenerator::generateDeleteCompositeLinks()
		$requete = 'DELETE FROM journal WHERE idmembre2 = '.$ididmembre2;
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Journal) {
			$oJournal = new Journal($object['id'],$object['idmembre1'],$object['action'],$object['idmembre2'],$object['date_heure']);
		} else {
			$oJournal = $object;
		}
		...
		if (database::getInstance()->executeRequete($oJournal->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Journal)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['journal']) && sizeof($_SESSION['journal']->compareTo($oJournal)) > 0) {
			$_SESSION['journal'] = $oJournal;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oJournal->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oJournal->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Journal();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeJournal); $i++) {
			$object = $this -> _aListeJournal[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Journal de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getJournalVide() {
