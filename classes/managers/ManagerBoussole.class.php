...
class ManagerBoussole {
	/** Instance de la classe (managerBoussole) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Boussole) */
	private static $_aListeBoussole = array();
	...
	public function getById($value=-1) {
		...
		return FactoryBoussole::getFromTableBoussole($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryBoussole::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Boussole) {
			$oBoussole = new Boussole($object['id'],$object['orientation']);
		} else {
			$oBoussole = $object;
		}
		...
		if (database::getInstance()->executeRequete($oBoussole->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Boussole)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['boussole']) && sizeof($_SESSION['boussole']->compareTo($oBoussole)) > 0) {
			$_SESSION['boussole'] = $oBoussole;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oBoussole->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oBoussole->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Boussole();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeBoussole); $i++) {
			$object = $this -> _aListeBoussole[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Boussole de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getBoussoleVide() {
