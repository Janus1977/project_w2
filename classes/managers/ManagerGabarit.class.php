...
class ManagerGabarit {
	/** Instance de la classe (managerGabarit) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Gabarit) */
	private static $_aListeGabarit = array();
	...
	public function getById($value=-1) {
		...
		return FactoryGabarit::getFromTableGabarit($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryGabarit::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Gabarit) {
			$oGabarit = new Gabarit($object['id'],$object['nom'],$object['x_depart'],$object['y_depart'],$object['x_arrivee'],$object['y_arrivee'],$object['taille']);
		} else {
			$oGabarit = $object;
		}
		...
		if (database::getInstance()->executeRequete($oGabarit->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Gabarit)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['gabarit']) && sizeof($_SESSION['gabarit']->compareTo($oGabarit)) > 0) {
			$_SESSION['gabarit'] = $oGabarit;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oGabarit->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oGabarit->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Gabarit();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeGabarit); $i++) {
			$object = $this -> _aListeGabarit[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Gabarit de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getGabaritVide() {
