...
class ManagerTroupes {
	/** Instance de la classe (managerTroupes) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Troupes) */
	private static $_aListeTroupes = array();
	...
	public function getById($value=-1) {
		...
		return FactoryTroupes::getFromTableTroupes($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryTroupes::getFromExtTableJoueur($value);
	}

	public function getFromExtTableIdmembre($ididmembre=-1) {
		//Generated by ManagerGenerator::generateGetFromTableFromFK()
		/* Appel de la methode de la Fabrique */
		return FactoryTroupes::getFromExtTableIdmembre($ididmembre);
	}

	public function deleteCompositeLinksFromIdmembre($ididmembre) {
		//Generated by ManagerGenerator::generateDeleteCompositeLinks()
		$requete = 'DELETE FROM troupes WHERE idmembre = '.$ididmembre;
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Troupes) {
			$oTroupes = new Troupes($object['id'],$object['nom'],$object['description'],$object['idmembre'],$object['date_creation'],$object['date_modification']);
		} else {
			$oTroupes = $object;
		}
		...
		if (database::getInstance()->executeRequete($oTroupes->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Troupes)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['troupes']) && sizeof($_SESSION['troupes']->compareTo($oTroupes)) > 0) {
			$_SESSION['troupes'] = $oTroupes;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oTroupes->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oTroupes->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Troupes();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeTroupes); $i++) {
			$object = $this -> _aListeTroupes[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Troupes de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getTroupesVide() {
