...
class ManagerArmeecomposition {
	/** Instance de la classe (managerArmeecomposition) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Armeecomposition) */
	private static $_aListeArmeecomposition = array();
	...
	public function getById($value=-1) {
		...
		return FactoryArmeecomposition::getFromTableArmeecomposition($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryArmeecomposition::getFromExtTableJoueur($value);
	}

	public function getFromExtTableArmee($idarmee=-1) {
		//Generated by ManagerGenerator::generateGetFromTableFromFK()
		/* Appel de la methode de la Fabrique */
		return FactoryArmeecomposition::getFromExtTableArmee($idarmee);
	}

	public function deleteCompositeLinksFromArmee($idarmee) {
		//Generated by ManagerGenerator::generateDeleteCompositeLinks()
		$requete = 'DELETE FROM armeecomposition WHERE armee = '.$idarmee;
	}

	public function getFromExtTableTroupe($idtroupe=-1) {
		//Generated by ManagerGenerator::generateGetFromTableFromFK()
		/* Appel de la methode de la Fabrique */
		return FactoryArmeecomposition::getFromExtTableTroupe($idtroupe);
	}

	public function deleteCompositeLinksFromTroupe($idtroupe) {
		//Generated by ManagerGenerator::generateDeleteCompositeLinks()
		$requete = 'DELETE FROM armeecomposition WHERE troupe = '.$idtroupe;
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Armeecomposition) {
			$oArmeecomposition = new Armeecomposition($object['id'],$object['armee'],$object['troupe']);
		} else {
			$oArmeecomposition = $object;
		}
		/* Appel de la methode delete de l'objet */
		/* Tout se passe dans une transaction ouverte plus haut */
			/* Execution de la requete */
		if (database::getInstance()->executeRequete($oArmeecomposition->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Armeecomposition)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['armeecomposition']) && sizeof($_SESSION['armeecomposition']->compareTo($oArmeecomposition)) > 0) {
			$_SESSION['armeecomposition'] = $oArmeecomposition;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oArmeecomposition->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oArmeecomposition->save());
	}

	public function findBy($champ,$data='') {
		//Generated by ManagerGenerator::generateFindBy()
		/* creation d'un objet de base de la classe */
		$object = new Armeecomposition();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeArmeecomposition); $i++) {
			$object = $this -> _aListeArmeecomposition[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Armeecomposition de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getArmeecompositionVide() {
		//Generated by ManagerGenerator::generateGetObjetVide()
		return new Armeecomposition();
	}
