...
class ManagerConnectes {
	/** Instance de la classe (managerConnectes) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Connectes) */
	private static $_aListeConnectes = array();
	...
	public function getById($value=-1) {
		...
		return FactoryConnectes::getFromTableConnectes($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryConnectes::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Connectes) {
			$oConnectes = new Connectes($object['id'],$object['membre'],$object['date_connexion']);
		} else {
			$oConnectes = $object;
		}
		...
		if (database::getInstance()->executeRequete($oConnectes->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Connectes)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['connectes']) && sizeof($_SESSION['connectes']->compareTo($oConnectes)) > 0) {
			$_SESSION['connectes'] = $oConnectes;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oConnectes->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oConnectes->save());
	}

	public function findBy($champ,$data='') {
		//Generated by ManagerGenerator::generateFindBy()
		/* creation d'un objet de base de la classe */
		$object = new Connectes();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeConnectes); $i++) {
			$object = $this -> _aListeConnectes[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Connectes de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getConnectesVide() {
		//Generated by ManagerGenerator::generateGetObjetVide()
		return new Connectes();
	}
