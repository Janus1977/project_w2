...
class ManagerArmees {
	/** Instance de la classe (managerArmees) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Armees) */
	private static $_aListeArmees = array();
	...
	public function getById($value=-1) {
		...
		return FactoryArmees::getFromTableArmees($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryArmees::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Armees) {
			$oArmees = new Armees($object['id'],$object['nom'],$object['description'],$object['idmembre'],$object['points'],$object['date_creation'],$object['date_modification']);
		} else {
			$oArmees = $object;
		}
		...
		if (database::getInstance()->executeRequete($oArmees->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Armees)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['armees']) && sizeof($_SESSION['armees']->compareTo($oArmees)) > 0) {
			$_SESSION['armees'] = $oArmees;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oArmees->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oArmees->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Armees();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeArmees); $i++) {
			$object = $this -> _aListeArmees[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Armees de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getArmeesVide() {
