...
class ManagerSol {
	/** Instance de la classe (managerSol) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Sol) */
	private static $_aListeSol = array();
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Sol) {
			$oSol = new Sol($object['id'],$object['nom'],$object['nb_cases_x'],$object['nb_cases_y'],$object['chemin']);
		} else {
			$oSol = $object;
		}
		...
		if (database::getInstance()->executeRequete($oSol->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Sol)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['sol']) && sizeof($_SESSION['sol']->compareTo($oSol)) > 0) {
			$_SESSION['sol'] = $oSol;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oSol->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oSol->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Sol();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeSol); $i++) {
			$object = $this -> _aListeSol[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Sol de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getSolVide() {
