...
class ManagerDesign {
	/** Instance de la classe (managerDesign) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Design) */
	private static $_aListeDesign = array();
	...
	public function getById($value=-1) {
		...
		return FactoryDesign::getFromTableDesign($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryDesign::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Design) {
			$oDesign = new Design($object['id'],$object['nom'],$object['description'],$object['chemin']);
		} else {
			$oDesign = $object;
		}
		...
		if (database::getInstance()->executeRequete($oDesign->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Design)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['design']) && sizeof($_SESSION['design']->compareTo($oDesign)) > 0) {
			$_SESSION['design'] = $oDesign;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oDesign->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oDesign->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Design();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListeDesign); $i++) {
			$object = $this -> _aListeDesign[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Design de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getDesignVide() {
