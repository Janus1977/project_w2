...
class ManagerPlateau {
	/** Instance de la classe (managerPlateau) */
	private static $_instance = null;

	/** Connexion a la base de donnees (database) */
	private static $_oConnexion = null;

	/** Liste des objet de la classe (Plateau) */
	private static $_aListePlateau = array();
	...
	public function getById($value=-1) {
		...
		return FactoryPlateau::getFromTablePlateau($value);
	}

	public function getByJoueurId($value) {
		//Generated by ManagerGenerator::generateGetByJoueurId()
		return FactoryPlateau::getFromExtTableJoueur($value);
	}
	...
	public function delete($object=array()) {
		...
		if (! $object instanceof Plateau) {
			$oPlateau = new Plateau($object['id'],$object['nom'],$object['description'],$object['chemin'],$object['nb_cases_x'],$object['nb_cases_y']);
		} else {
			$oPlateau = $object;
		}
		...
		if (database::getInstance()->executeRequete($oPlateau->delete())) {
			/* Requete OK */
			return true;
		} else {
			/* Requete NOK lancement d'une exception PDO */
			throw new PDOException('Erreur sur delete (Plateau)');
		}
	}
	...
	public function update($object=array()) {
		...
		if (!empty($_SESSION['plateau']) && sizeof($_SESSION['plateau']->compareTo($oPlateau)) > 0) {
			$_SESSION['plateau'] = $oPlateau;
		}
		/* on update car les objets sont different */
		return database::getInstance()->executeRequete($oPlateau->update());
	}
	...
	public function save($object=array()) {
		...
		return database::getInstance()->executeRequete($oPlateau->save());
	}
	...
	public function findBy($champ,$data='') {
		...
		$object = new Plateau();
		$resultat = array();
		for ($i = 0; $i < sizeof($this -> _aListePlateau); $i++) {
			$object = $this -> _aListePlateau[$i];
		...
		if (sizeof($resultat) > 0) {
			...
			//n'existe pas
			return "Pas de Plateau de ".strtolower($champ)." '".$data."'";
		}
	}

	public function getPlateauVide() {
